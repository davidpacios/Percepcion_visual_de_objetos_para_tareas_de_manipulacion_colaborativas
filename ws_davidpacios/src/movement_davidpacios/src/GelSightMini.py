...
class GelSightMini(object):
    ...
    def __del__(self):
        print("[GelSight Mini]      Releasing camera")
        self.cap.release()
    ...
    def calibrate(self):
        print("[GelSight Mini]      Calibrating: Don't touch the gelsight...")
        first_ret = False
        t0 = rospy.get_time()
        throw_away = 10
        ...
        print("[GelSight Mini]      Calibration Complete: Default max distance =",self.avg_dist/255*100)
    ...
    def get_max_distance(self):
        if (self.cap.isOpened()):
            ...
            print("[GelSight Mini]      Error Capture is closed")
            return
        
    def loop(self):

        pub = rospy.Publisher('gelsight_pixel_intensity_difference', Float32, queue_size=0)

        while True:

            if not self.freeze:
                pixel_intensity_difference = self.get_max_distance()
                pub.publish(Float32(pixel_intensity_difference))
...
if __name__ == '__main__':
    rospy.init_node('gelsight_mini_server')
    server = GelSightMini('gelsight_mini_action')
    rospy.spin()